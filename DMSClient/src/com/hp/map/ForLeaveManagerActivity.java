...
public class ForLeaveManagerActivity extends MainMenuActivity implements OnClickListener, DateWatcher {
	...
	public ListView forLeaveListView;

	private EditText id_search;
	private EditText name_search;

	public ForLeaveArrayAdapter adapter;
	...
	public static ForLeave selectedValue;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		id_search.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence cs, int arg1, int arg2,
				...
				ForLeaveManagerActivity.this.adapter.getFilter().filter(cs);
			}
...
//            return true;
	...
	public void addListView() {
		...
		adapter = new ForLeaveArrayAdapter(this,
				android.R.layout.simple_list_item_1, getForLeaveList(filter));
		forLeaveListView.setAdapter(adapter);

		forLeaveListView.setOnItemClickListener(new OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> a, View v, int position,
					long id) {
				System.out.println("Click!");
				//selectedValue = (ForLeave) forLeaveListView.getAdapter().getItem(position);
...
//	}
	...
	public void getSetLunchList(){
		...
		GetForLeaveTask getData = new GetForLeaveTask(context, getList, Rest.mStaff.getId(), this );
        getData.execute();
	        
	}
	
	public List<ForLeave> getForLeaveList(boolean filter){
		if(filter == false){
			return ForLeaveAPI.forLeaveList;
		}
		
		else
			return forLeaveListFilter;
	}
	public void onClick(View v) {
		for (int i = 0; i < getForLeaveList(filter).size(); i++)
			if (v == id[i]) {
	...
	public void button_filter(View view){
		...
		for(int i = 0; i < ForLeaveAPI.forLeaveList.size(); i++){
			
			if(ForLeaveAPI.forLeaveList.get(i).getTimeAt() == null)
				continue;
			Date compare = ForLeaveAPI.forLeaveList.get(i).getTimeAt();
				
			System.out.println(compare);
			if(checkDate(compare, startDate, endDate)){
				
				forLeaveListFilter.add(ForLeaveAPI.forLeaveList.get(i));
				//System.out.println(CalendarAPI.calendarList.get(i).getStt());
			}
		}
		
		adapter = new ForLeaveArrayAdapter(this,
				android.R.layout.simple_list_item_1, forLeaveListFilter);
		forLeaveListView.setAdapter(adapter);
		//onResume();
	...
	public void onDateChanged(ForLeave c) {
		// TODO Auto-generated method stub
